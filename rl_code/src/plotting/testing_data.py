...
for ep in range(len(file_names)):
    ...
    # ax2.plot(data['gsp_heading'], c= 'lightblue')
    # # Look to see if there are NUM ROBOTS GSP headings... 
    ...
    # # plt.ylim(-1, 1)
    # ax2.plot(last_10_gsp_index, last_10_gsp, c='b', label='GSP')
    # ax2.set_ylabel("Heading Change (rad)", fontsize = 20.0)
    # ax2.set_xlabel("Timestep", fontsize =20.0)
    # plt.plot(np.radians(cyl_heading_diff), c='r', label='Actual')
    # ax2.legend()
