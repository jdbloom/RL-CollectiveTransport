...
class TestAgent(unittest.TestCase):
    ...
    def test_mailbox(self):
        ...
        for i in range(agent.num_agents):
            self.assertEqual(len(agent.mailbox.inbox[i]), 2)
            for message in agent.mailbox.inbox[i]:
                self.assertEqual(message.contents, -1)
        # Test communication
        agent.schedule_message_to_all_contacts(0, 1)
        agent.schedule_message_to_all_contacts(1, 2)
        agent.schedule_message_to_all_contacts(2, 3)
        agent.schedule_message_to_all_contacts(3, 4)
        # Test outbox messages (already checked neighbors in test_make_comms)
        for i in range(agent.num_agents):
            self.assertEqual(len(agent.mailbox.outbox[i]), 2)
            for j in range(2):
                self.assertEqual(agent.mailbox.outbox[i][j].contents, (i+1))
        # Test carry_mail
        agent.carry_mail()
        for i in range(agent.num_agents):
            self.assertEqual(len(agent.mailbox.outbox[i]), 0)
        # Test inboxes
        self.assertEqual(agent.mailbox.inbox[0][0].contents, 2)
        self.assertEqual(agent.mailbox.inbox[0][1].contents, 4)
        self.assertEqual(agent.mailbox.inbox[1][0].contents, 1)
        self.assertEqual(agent.mailbox.inbox[1][1].contents, 3)
        self.assertEqual(agent.mailbox.inbox[2][0].contents, 2)
        self.assertEqual(agent.mailbox.inbox[2][1].contents, 4)
        self.assertEqual(agent.mailbox.inbox[3][0].contents, 1)
        self.assertEqual(agent.mailbox.inbox[3][1].contents, 3)
        # Test get_agent_incomming_commiunications
        msg1 = agent.get_agent_incoming_communications(0)
        msg2 = agent.get_agent_incoming_communications(1)
        msg3 = agent.get_agent_incoming_communications(2)
        msg4 = agent.get_agent_incoming_communications(3)
        # Agent 0 Left msg == 4 (0 0 0 1)
        self.assertEqual(msg1.left_msg[0], 0)
        self.assertEqual(msg1.left_msg[1], 0)
        self.assertEqual(msg1.left_msg[2], 0)
        self.assertEqual(msg1.left_msg[3], 1)
        # Agent 0 Right msg == 2 (0 1 0 0)
        self.assertEqual(msg1.right_msg[0], 0)
        self.assertEqual(msg1.right_msg[1], 1)
        self.assertEqual(msg1.right_msg[2], 0)
        self.assertEqual(msg1.right_msg[3], 0)
        # Agent 1 Left msg == 1 (1 0 0 0)
        self.assertEqual(msg2.left_msg[0], 1)
        self.assertEqual(msg2.left_msg[1], 0)
        self.assertEqual(msg2.left_msg[2], 0)
        self.assertEqual(msg2.left_msg[3], 0)
        # Agent 1 Right msg == 3 (0 0 1 0)
        self.assertEqual(msg2.right_msg[0], 0)
        self.assertEqual(msg2.right_msg[1], 0)
        self.assertEqual(msg2.right_msg[2], 1)
        self.assertEqual(msg2.right_msg[3], 0)
        # Agent 2 Left msg == 2 (0 1 0 0)
        self.assertEqual(msg3.left_msg[0], 0)
        self.assertEqual(msg3.left_msg[1], 1)
        self.assertEqual(msg3.left_msg[2], 0)
        self.assertEqual(msg3.left_msg[3], 0)
        # Agent 2 Right msg == (0 0 0 1)
        self.assertEqual(msg3.right_msg[0], 0)
        self.assertEqual(msg3.right_msg[1], 0)
        self.assertEqual(msg3.right_msg[2], 0)
        self.assertEqual(msg3.right_msg[3], 1)
        # Agent 3 Left msg == 3 (0 0 1 0)
        self.assertEqual(msg4.left_msg[0], 0)
        self.assertEqual(msg4.left_msg[1], 0)
        self.assertEqual(msg4.left_msg[2], 1)
        self.assertEqual(msg4.left_msg[3], 0)
        # Agent 3 Right msg == 1 (1 0 0 0)
        self.assertEqual(msg4.right_msg[0], 1)
        self.assertEqual(msg4.right_msg[1], 0)
        self.assertEqual(msg4.right_msg[2], 0)
        self.assertEqual(msg4.right_msg[3], 0)
    ...
    def test_memory(self):
        ...
        for i in range(agent.batch_size):
            ...
            comms = np.random.rand(2*agent.alphabet_size)
            state = np.concatenate((observation, comms))
            action_num = np.random.choice(agent.num_ops_per_action**agent.num_actions)
            action = np.random.uniform(low = -agent.min_max_action, high = agent.min_max_action, size = (2,))
            message = np.random.choice(agent.alphabet_size)
            reward = np.random.random()
            state_ = state
            done = False
            agent.store_transition(state, (action_num, action), reward, state_, done)
            agent.store_comms_transition(state, (message, None), reward, state_, done)
        s, a, r, s_, d = agent.sample_memory(agent.q_eval)
